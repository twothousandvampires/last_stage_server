...
export default class GrimPileCast {
    ...
    constructor(private inc_effect: boolean, private add_resistace: boolean){

    }

    canUse(pile: Enemy){
        return pile.level.time - this.last_used_time >= this.cooldown && pile.target && Func.distance(pile, pile.target) <= this.radius
    }
