...
export default class InfernalGaze extends Mastery {
    constructor(){
        ...
        this.description = 'When the ability is activated, there is a chance to ignite all enemies in front of you.'
    }
    ...
    trigger(player: Character, ability: Ability){
        if(ability.getMasteryChance()){
            ...
            let f = player.level.enemies.filter(elem => !elem.is_dead && Func.distance(player, elem) <= 30 && Func.checkAngle(player, elem, gaze_angle, 1.2))
