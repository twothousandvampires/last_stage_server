...
export class MasterManifestation extends Manifistation {
    ...
    activate(): void {
        ...
        let targets = this.level.players.filter(elem => !elem.is_dead && Func.elipseCollision(elem.getBoxElipse(), box))
    ...
    giveReward(){
        ...
        let filtered = list.filter(elem => elem.canUse(this.activated_by) && elem.cost <= value && (!elem.ascend || elem.ascend <= this.activated_by.ascend_level))
