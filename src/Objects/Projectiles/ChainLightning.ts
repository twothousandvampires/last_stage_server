...
export class ChainLightning extends Projectiles{
    ...
    act(): void { 
        ...
        for(let i = 0; i < players.length; i++){
            ...
            if(!p.is_dead && Func.elipseCollision(this.getBoxElipse(), p.getBoxElipse()) && !this.hited.includes(p.id)){
        ...
        for(let i = 0; i < enemies.length; i++){
            let e = enemies[i]
            if(!e.is_dead && Func.elipseCollision(this.getBoxElipse(), e.getBoxElipse()) && !this.hited.includes(e.id)){
    ...
    impact(){
        ...
        this.level.players.forEach(p => {
            if(!p.is_dead && Func.distance(this, p) <= radius && p != this.owner && !this.hited.includes(p.id)){
                targets.push(p)
            }
        })
